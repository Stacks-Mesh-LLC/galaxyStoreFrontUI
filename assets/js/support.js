...
(() => {
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      const urls = [
        COMPONENT_DIR + "/" + encodeURIComponent(name) + ".html",
        COMPONENT_DIR + "/" + kebab + ".html",
        COMPONENT_DIR + "/" + encodeURIComponent(name) + ".dc.html",
        COMPONENT_DIR + "/" + kebab + ".dc.html"
      ];
      const tryFetch = (i) => {
        if (i >= urls.length) {
          console.error(
            "[dc-runtime] sibling fetch for <" + name + "/> failed across all candidates: " + urls.join(", ")
          );
          return Promise.resolve("");
        }
        return fetch(urls[i]).then((res) => {
      ...
      tryFetch(0).then((t) => {
        ...
        if (!parsed) {
          console.error(
            "[dc-runtime] sibling fetch for <" + name + "/> has no <x-dc> block — not a Design Component."
          );
          return;
        }
